...
export const getUserEpmaps = async (email: string) => {
  try {
    const client = await createClientAsync(EPMAPS_SOAP_URL!, {
      endpoint: EPMAPS_ENDPOINT,
      wsdl_options: {
        auth: {
          username: EPMAPS_SOAP_USER,
          password: EPMAPS_SOAP_PASSWORD,
        },
      },
    });
    client.setSecurity(new BasicAuthSecurity(EPMAPS_SOAP_USER!, EPMAPS_SOAP_PASSWORD!));
